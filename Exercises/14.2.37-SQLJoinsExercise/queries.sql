...
SELECT
	first_name,
	last_name,
	COALESCE(COUNT(vehicles.owner_id), 0)
FROM
	owners
	left JOIN vehicles ON owners.id = vehicles.owner_id
GROUP BY
...
-- average price for each of the cars as integers.
-- Display the owners first_name , last_name, average price
-- and count of vehicles. The first_name should be ordered in
-- descending order. Only display results with more than one
-- vehicle and an average price greater than 10000. Your output
